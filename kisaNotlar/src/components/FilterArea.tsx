...
type filterAreaProps = {
  ...
  const FilterArea = (props: filterAreaProps) => {
    const { keyword, setKeyword, sortByAscending, sortByDescending, setFilterWord, isSelectZA, isSelectAZ, setIsSelectZA, setIsSelectAZ } = props;
  
    return (<View>
      <TextInput
        style={styles.textInput}
        onChangeText={
          (keyword: string) => {setKeyword(keyword);
        }}
        value={keyword}
        ...
        <View style={styles.sortingButtonView}>
          ...
          >
            <Text style={[styles.sortingButtonText, 
              { color: isSelectAZ ? "#447970" : "white" }]}>Artan</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.sortingButton, 
          ...
          >
            <Text style={[styles.sortingButtonText, 
              { color: isSelectZA ? "#447970" : "white" }]}>Azalan</Text>
          </TouchableOpacity>
        </View>
      </View>
      <TouchableOpacity
